...
//#define DEBUG_A100_PLATFORM

#ifdef DEBUG_A100_PLATFORM
#define DEBUG printf
#else
#define DEBUG(...)
#endif
...
void adjust_ranges_a100(struct emulator_config *cfg) {
    cfg->mapped_high = 0;
    cfg->mapped_low = 0;
    cfg->custom_high = 0;
    cfg->custom_low = 0;
    ...
    printf("[A100] Platform custom range: %.8X-%.8X\n", cfg->custom_low, cfg->custom_high);
    printf("[A100] Platform mapped range: %.8X-%.8X\n", cfg->mapped_low, cfg->mapped_high);
}


int setup_platform_a100(struct emulator_config *cfg) {
    printf("Performing setup for A100 platform.\n");
...
void setvar_a100(struct emulator_config *cfg, char *var, char *val) {
    if (!var)
        return;
...
void handle_ovl_mappings_a100(struct emulator_config *cfg) {
    int32_t index = -1;
    ...
    adjust_ranges_a100(cfg);
}

void handle_reset_a100(struct emulator_config *cfg) {
    ...
    handle_ovl_mappings_a100(cfg);
}

void shutdown_platform_a100(struct emulator_config *cfg) {
    ...
    printf("[A100] Platform shutdown completed.\n");
}

void create_platform_a100(struct platform_config *cfg, char *subsys) {
    cfg->register_read = NULL;
    cfg->register_write = NULL;
    cfg->custom_read = NULL;
    cfg->custom_write = NULL;
    cfg->platform_initial_setup = setup_platform_a100;
    cfg->handle_reset = handle_reset_a100;
    cfg->shutdown = shutdown_platform_a100;

    cfg->setvar = setvar_a100;
    cfg->id = PLATFORM_MAC;
